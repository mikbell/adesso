...
// Importa componenti, dati e la funzione di fetch
...
const EditProduct = () => {
    const { productId } = useParams(); // 1. Recupera l'ID del prodotto dall'URL
    ...
    // 2. Carica i dati del prodotto al montaggio del componente
    useEffect(() => {
        const loadProduct = async () => {
            try {
                const fetchedProduct = await fetchProductById(productId);

                // 3. Pre-compila lo stato con i dati esistenti
                setProductData({
                    ...fetchedProduct,
                    // Assicurati che categoria e stato siano gli oggetti corretti, non solo i valori
                    category: categories.find(c => c.name === fetchedProduct.category) || categories[0],
                    status: statuses.find(s => s.value === fetchedProduct.status.toLowerCase()) || statuses[0],
                });
                setImagePreviews(fetchedProduct.images || []);
            } catch (error) {
                console.error("Prodotto non trovato:", error);
                // Opzionale: reindirizza se il prodotto non esiste
    ...
    // ... Le funzioni handler (handleChange, handleImageChange, etc.) rimangono le stesse ...
    ...
    const handleSubmit = (e) => {
        ...
        console.log("Dati aggiornati da inviare:", dataToSend);
        setTimeout(() => {
            setIsSaving(false);
            alert('Prodotto aggiornato!');
            navigate(`/seller/dashboard/products/${productId}/view`); // Torna alla pagina di dettaglio
        }, 1500);
    };

    if (loading) {
        return <div className="p-6 text-center">Caricamento prodotto...</div>;
    }
